...
@testset "single dimensional neighborhood tests" begin
    ...
    # edge case tests
    foreach(t -> (@test AutoBVA.edgecase((IntSubtractionOperator()), t)), typemin.([Bool, UInt8, UInt32]))
    foreach(t -> (@test AutoBVA.edgecase((IntAdditionOperator()), t)), typemax.([Bool, UInt8, UInt32]))

    foreach(t -> (@test false == AutoBVA.edgecase((IntSubtractionOperator()), t)), [true, UInt8(3), UInt32(3)])
    foreach(t -> (@test false == AutoBVA.edgecase((IntAdditionOperator()), t)), [false, UInt8(3), UInt32(3)])
